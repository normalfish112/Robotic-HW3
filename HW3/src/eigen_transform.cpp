...
Eigen::Vector3d Quaternion2Euler(Eigen::Quaterniond Q)
{
    ...
    Euler.x() = atan2(2 * (Q.x() * Q.y() + Q.w() * Q.z()), (1 - 2 * (Q.y() * Q.y() + Q.z() * Q.z())));
    return Euler;
}
...
int main(int argc, char **argv)
{
    ...
    //std::cout << "Current point position " << tmp_point.transpose() << std::endl << std::endl;
    while (ros::ok())
    {
...
//        std::cout << Q_tmp.coeffs().transpose() << std::endl;
	...
	P.w() = 0;
	P.vec() = world_point;
	Eigen::Quaterniond RotatedP = Q_tmp*P*Q_tmp.inverse();
	world_point = RotatedP.vec();
